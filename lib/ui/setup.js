  ...
  try {
    ...
    const step1 = await inquirer.prompt([
      ...
      },
      {
        type: 'input',
        name: 'apiKeyEnv',
        message: 'API key ENV var name (recommended; users supply keys):',
        when: (a) => !['ollama', 'custom'].includes(a.provider),
        default: (a) => providers.find((p) => p.id === a.provider)?.defaultApiKeyEnv || current.apiKeyEnv || '',
        validate: (v) => (v && v.length ? true : 'Provide an env var name (e.g., OPENAI_API_KEY)')
      },
      {
        type: 'confirm',
        name: 'storeApiKey',
        message: 'Store API key in ~/.protoforge/config.json? (NOT recommended)',
        when: (a) => !['ollama', 'custom'].includes(a.provider),
        default: false
      },
      {
        type: 'password',
        name: 'apiKey',
        message: 'API key (will be stored locally):',
        when: (a) => !['ollama', 'custom'].includes(a.provider) && a.storeApiKey === true,
        validate: (v) => (v && v.length ? true : 'API key is required if you choose to store it')
      },
      {
        type: 'input',
        name: 'temperature',
